...
$db=mysqli_connect('localhost','root','','moviedb') or die("could not connect");
  $query="SELECT * FROM actor,director,genre,movies,production,rating WHERE movies.mov_id=rating.mov_id AND rating>3 AND movies.act_id=actor.act_id AND movies.dir_id=director.dir_id AND genre.mov_id=movies.mov_id AND movies.pro_id=production.pro_id order by rating desc ;";
  $sql=mysqli_query($db,$query);
  $num=mysqli_num_rows($sql);
  if ($num<1) {
  header("location:toprated.php?message=not_found");
  }
  else{
?>
<html>
